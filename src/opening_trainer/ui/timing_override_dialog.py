...
class TimingOverrideDialog:
    ...
    def refresh(self) -> None:
        ...
        self.diagnostics_var.set(
            ...
            f"Last attempted context key: {d.last_attempted_context_key or 'n/a'}\n"
            f"Last fallback keys attempted: {list(d.last_fallback_keys_attempted)}\n"
            f"Last matched context key: {d.last_matched_context_key or 'n/a'}\n"
            f"Last move-pressure profile: {d.last_move_pressure_profile_id or 'n/a'}\n"
            f"Last think-time profile: {d.last_think_time_profile_id or 'n/a'}\n"
            f"Last sampled think time: {d.last_sampled_think_time_seconds if d.last_sampled_think_time_seconds is not None else 'n/a'}\n"
            f"Last visible delay applied: {d.last_visible_delay_applied_seconds if d.last_visible_delay_applied_seconds is not None else 'none'}\n"
            f"Last visible delay reason: {d.last_visible_delay_reason}\n"
            f"Last opponent source path: {d.last_opponent_source_path or 'n/a'}\n"
            f"Review predecessor bypassed by override: {d.review_predecessor_bypassed_by_override}"
        )
