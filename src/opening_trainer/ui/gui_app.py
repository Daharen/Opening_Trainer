...
class OpeningTrainerGUI:
    ...
    def __init__(self, session: TrainingSession | None = None, runtime_context: RuntimeContext | None = None):
        ...
        self.opening_locked_enabled_check = ttk.Checkbutton(
            self.control_strip,
            text='Opening-locked',
            variable=self.opening_locked_enabled_var,
            command=self._on_manual_contract_changed,
        )
        self.opening_locked_enabled_check.grid(row=0, column=16, sticky='w', padx=(0, 8))
    ...
    def _refresh_top_control_strip(self) -> None:
        ...
        artifact_available = bool(artifact_status is not None and getattr(artifact_status, "loaded", False))
        ...
        if hasattr(self, "opening_locked_opening_var"):
            self.opening_locked_opening_var.set(self.session.settings.selected_opening_name or "")
        if hasattr(self, "opening_locked_enabled_check"):
            self.opening_locked_enabled_check.configure(state=('normal' if artifact_available else 'disabled'))
    ...
    def _training_depth_summary(self) -> str:
        ...
        if artifact_status is not None:
            opening_selected = self.session.settings.selected_opening_name or "n/a"
            opening_requested = "on" if self.session.settings.opening_locked_mode_enabled else "off"
            effective = bool(getattr(getattr(self.session, "opening_locked_state", None), "enabled", False))
            if not getattr(artifact_status, "loaded", False):
                summary = "unavailable: artifact missing"
            elif not self.session.settings.selected_opening_name:
                summary = "ready: choose an opening" if self.session.settings.opening_locked_mode_enabled else "off"
            elif effective:
                summary = f"selected: {self.session.settings.selected_opening_name}"
            else:
                summary = f"off; remembered opening: {self.session.settings.selected_opening_name}"
            opening_lock_text = (
                f' | Opening-lock artifact={"yes" if artifact_status.loaded else "no"}'
                f' requested={opening_requested}'
                f' selected={opening_selected}'
    ...
    def _apply_theme(self) -> None:
        ...
        if dark:
            if not hasattr(self, '_default_ttk_theme_name'):
                try:
                    self._default_ttk_theme_name = style.theme_use()
                except tk.TclError:
                    self._default_ttk_theme_name = None
            try:
            ...
            default_theme_name = getattr(self, '_default_ttk_theme_name', None)
            if default_theme_name:
                try:
                    if style.theme_use() != default_theme_name:
                        style.theme_use(default_theme_name)
                except tk.TclError:
                    pass
        ...
        style.map(
            'TopStrip.TCombobox',
            fieldbackground=[('readonly', palette['field_bg']), ('disabled', palette['surface_bg'])],
            background=[('readonly', palette['field_bg']), ('disabled', palette['surface_bg'])],
            foreground=[('readonly', palette['text_fg']), ('disabled', palette['muted_fg'])],
            selectbackground=[('readonly', palette['select_bg'])],
            selectforeground=[('readonly', palette['text_fg'])],
            arrowcolor=[('readonly', palette['text_fg']), ('disabled', palette['muted_fg'])],
        )
        style.configure(
            'InspectorFilter.TCombobox',
            fieldbackground=palette['field_bg'],
            background=palette['field_bg'],
            foreground=palette['text_fg'],
            arrowcolor=palette['text_fg'],
        ...
        style.configure('ReviewPanel.TButton', background=palette['button_bg'], foreground=palette['text_fg'], bordercolor=palette['border_color'])
        style.map(
            'ReviewPanel.TButton',
            background=[('active', palette['button_active_bg']), ('pressed', palette['button_active_bg']), ('disabled', palette['surface_bg'])],
