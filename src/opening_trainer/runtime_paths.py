...
from .install_layout import default_mutable_app_root, read_installed_app_manifest
...
def resolve_runtime_paths(mode: RuntimeMode, *, repo_root: Path, workspace_root: Path) -> ResolvedRuntimePaths:
    if mode is RuntimeMode.CONSUMER:
        ...
        app_payload_root = Path(installed_manifest["mutable_app_root"]) if installed_manifest and installed_manifest.get("mutable_app_root") else default_mutable_app_root()
