...
def _create_real_schema_db(
    ...
    *,
    ...
    admissions=(),
    explanations=(),
    include_root_summary_count_columns: bool = True,
):
    ...
    if include_root_summary_count_columns:
        conn.execute(
            """
...
def test_legacy_engine_fail_wording_only_when_final_outcome_is_fail(tmp_path):
    service = PracticalRiskReconciledService(None, expected_time_control_id="600+0")
    result = _eval(service)
    assert result.accepted is False
    assert "outside engine tolerance" in result.reason_text
...
def test_invalid_failure_explanations_schema_fails_activation_cleanly(tmp_path):
    db = tmp_path / "broken_failure.sqlite"
    conn = sqlite3.connect(db)
    conn.execute("CREATE TABLE artifact_metadata(key TEXT, value TEXT)")
    conn.execute(
        """
    ...
    service = PracticalRiskReconciledService(db, expected_time_control_id="600+0")
    assert service.active is False
    assert "artifact schema mismatch in failure_explanations" in (service.activation_error or "")
...
def test_incompatible_root_summary_schema_is_non_fatal_and_inspectable(tmp_path):
    ...
    assert "missing optional count columns" in (service.root_summary_activation_warning or "")


def test_service_activates_and_rescues_when_root_summary_counts_missing(tmp_path):
    board = chess.Board()
    position_key = _position_key(board)
    ...
    _create_real_schema_db(
        ...
        include_root_summary_count_columns=False,
    )
...
def test_rescued_outcome_reason_text_does_not_keep_engine_fail_wording(tmp_path):
    board = chess.Board()
    position_key = _position_key(board)
    db = tmp_path / "rescued_text.sqlite"
    _create_real_schema_db(
        db,
        admissions=[
            (position_key, "1400-1600", "e2e4", 0, 0, 1, 1, "local", "local", "reconciled", "reconciled", "good", "stafford line", "1400-1600")
    ...
    result = _eval(service, requested_band_id="1200-1400")
    assert result.accepted is True
    assert result.canonical_judgment.name == "BETTER"
